...
public class ConfluenceAuthService implements IConfluenceAuthService {
	...
	@Override
	public AccessTokenResponse fetchOAuthToken() throws FatalException {
		AccessTokenRequest accessTokenRequest = new AccessTokenRequest(grantType, clientId, clientSecret);
		AccessTokenResponse accessTokenResponse = webClient.post().uri(baseUrl+tokenUri).contentType(MediaType.APPLICATION_JSON)
